...
class VectorRetriever:
    ...
    def _get_embedding(self, text: str, max_retries: int = 3):
        ...
        if self.embedding_provider == "openai":
            ...
            # 确保 API Key 已设置（优先使用已设置的，否则从环境变量读取）
            ...
            if not dashscope.api_key:
                raise RuntimeError(
                    "DashScope API Key 未设置！请按以下步骤操作：\n"
                    "1. 访问 https://dashscope.console.aliyun.com/apiKey 获取 API Key\n"
                    "2. 将项目根目录下的 env 文件重命名为 .env\n"
                    "3. 在 .env 文件中将 DASHSCOPE_API_KEY 设置为你的 API Key\n"
                    "4. 重启 Streamlit 应用"
                )
            ...
            for attempt in range(max_retries):
                ...
                # 防护：rsp 可能为 None
                if rsp is None:
                    last_error = RuntimeError("DashScope embedding API 返回了 None，请检查 API Key 或网络连接")
                    if attempt < max_retries - 1:
                        print(f"[Embedding] 第 {attempt + 1} 次尝试失败（rsp=None），{1.5 ** (attempt + 1):.0f} 秒后重试...")
                        time_module.sleep(1.5 ** (attempt + 1))
                        continue
                    raise last_error

                # 兼容 dashscope 返回格式
                output = rsp.get('output') if isinstance(rsp, dict) else None
                if output is None:
                    status_code = rsp.get('status_code', 'N/A') if isinstance(rsp, dict) else 'N/A'
                    last_error = RuntimeError(
                        f"DashScope embedding API 返回异常（output 为空，status_code={status_code}），"
                        f"完整响应: {rsp}"
                    )
                    if attempt < max_retries - 1:
                        print(f"[Embedding] 第 {attempt + 1} 次尝试失败（output=None），{1.5 ** (attempt + 1):.0f} 秒后重试...")
                        time_module.sleep(1.5 ** (attempt + 1))
                        continue
                    raise last_error
                ...
                if 'embeddings' in output:
                    ...
                    if emb.get('embedding') is None or len(emb.get('embedding', [])) == 0:
            ...
            # 理论上不会走到这里
            raise last_error or RuntimeError("DashScope embedding API 调用失败")
        else:
            raise ValueError(f"不支持的 embedding provider: {self.embedding_provider}")
    ...
    def retrieve_by_company_name(self, company_name: str, query: str, llm_reranking_sample_size: int = None, top_n: int = 3, return_parent_pages: bool = False) -> List[Tuple[str, float]]:
        ...
        # 估算 total_lines → 用于从行号推算页码
        total_chunks = len(chunks)
        total_lines_est = chunks[-1].get('lines', [0, 0])[1] if total_chunks > 0 and 'lines' in chunks[-1] else 0
        lines_per_page = max(total_lines_est / max(total_chunks, 1) * 30, 1)  # 粗略估算行/页比
        # 更可靠的方式：用最后一行除以总块数再乘以块行数，但这里直接用固定 50 行/页
        lines_per_page = 50
        ...
        for distance, index in zip(distances[0], indices[0]):
            ...
            # 获取页码（1-based）：优先 chunk.page，其次从 lines 估算
            chunk_page = chunk.get("page")
            if chunk_page is None and "lines" in chunk:
                chunk_page = (chunk["lines"][0] - 1) // lines_per_page + 1  # 1-based
            chunk_page = chunk_page if chunk_page is not None else 1

            parent_page = None
            if pages:
                parent_page = next((page for page in pages if page["page"] == chunk_page), None)
