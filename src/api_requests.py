...
class BaseDashscopeProcessor:
    ...
    def send_message(
        ...
        **kwargs
        ...
        # 防护：response 可能为 None
        if response is None:
            raise RuntimeError("DashScope Generation API 返回了 None，请检查 API Key 或网络连接")
        # 检测 API 错误（如 403 AccessDenied, 401 等）
        status_code = response.status_code if hasattr(response, 'status_code') else response.get('status_code', 0) if isinstance(response, dict) else 0
        if status_code and status_code != 200:
            ...
            raise RuntimeError(
                f"DashScope API 返回错误 (status_code={status_code}): {error_msg}\n"
                f"请检查：1) API Key 是否正确  2) 是否已开通 {model} 模型\n"
                f"开通地址: https://dashscope.console.aliyun.com/model\n"
                f"完整响应: {response}"
            )
        ...
        if hasattr(response, 'output') and response.output is not None and hasattr(response.output, 'choices'):
        ...
        self.response_data = {
            "model": model,
            "input_tokens": response.usage.input_tokens if hasattr(response, 'usage') and response.usage is not None and hasattr(response.usage, 'input_tokens') else None,
            "output_tokens": response.usage.output_tokens if hasattr(response, 'usage') and response.usage is not None and hasattr(response.usage, 'output_tokens') else None
        }
