...
class QuestionsProcessor:
    ...
    def _format_retrieval_results(self, retrieval_results) -> str:
        ...
        for result in retrieval_results:
            page_number = result['page']  # retrieval 已经是 1-based
            text = result['text']
    ...
    def _extract_companies_from_subset(self, question_text: str) -> list[str]:
        ...
        found_companies = []
        company_names_raw = self.companies_df['company_name'].dropna().unique()
        if len(company_names_raw) == 0:
            raise ValueError("subset.csv 中 company_name 列无有效数据")
        company_names = sorted(company_names_raw, key=len, reverse=True)
